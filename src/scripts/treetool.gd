...
func _extrude_loop(loop: PackedVector3Array,
	...
	for es in extrusion_steps:
		...
		for tri in loop.size():
			...
			var v1 = (loop[i0] * curve_multiplier_0) + offset_0
			var v2 = (loop[i1] * curve_multiplier_0) + offset_0
			var v3 = (loop[i0] * curve_multiplier_1 * close) + lerp(offset_0,
																	offset_1,
																	close)
			...
			v1 = (loop[i1] * curve_multiplier_0) + offset_0
			v2 = (loop[i1] * curve_multiplier_1 * close) + lerp(offset_0,
																	offset_1,
																	close)
			v3 = (loop[i0] * curve_multiplier_1 * close) + lerp(offset_0,
																	offset_1,
																	close)
			vertex_array.append(v1)
			vertex_array.append(v2)
